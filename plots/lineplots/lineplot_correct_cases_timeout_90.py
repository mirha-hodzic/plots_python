...
for tool in tools:
    query = f"SELECT avg_result_runtime FROM _run_group WHERE all_correct=1 AND tool='{tool}' AND run_timeout= 90 ORDER BY avg_result_runtime ASC"
...
plt.title("Line plot - Correct cases for each tool when run timeout equals 90")
plt.grid(True)
plt.legend()
