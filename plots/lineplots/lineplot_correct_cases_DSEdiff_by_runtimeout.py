...
def generate_plot10():
    ...
    for run_timeout in run_timeouts:
        query = f"SELECT avg_result_runtime FROM _run_group WHERE all_correct=1 AND tool='DSE-diff' AND run_timeout={run_timeout} ORDER BY avg_result_runtime ASC"
    ...
    ax.set_title("Line plot - Correct cases for tool 'DSE-diff' sorted by run timeout")
    ax.set_xscale('log')
    ax.grid(True)
    ax.legend()
