...
def generate_plot7():
    ...
    for run_timeout in run_timeouts:
        query = f"SELECT avg_result_runtime FROM _run_group WHERE all_correct=1 AND tool='ARDiff-base' AND run_timeout={run_timeout} ORDER BY avg_result_runtime ASC"
    ...
    ax.set_title("Line plot - Correct cases for tool 'ARDiff-base' sorted by run timeout")
    ax.set_xscale('log')
    ax.grid(True)
    ax.legend()
